...
/* LV_USE_EGL supplies the EGL entry points and LV_USE_OPENGLES the GL ones, and this
 * driver calls through both. */
...
lv_display_t * lv_opengles_pbuffer_create(int32_t hor_res, int32_t ver_res)
{
    ...
    ctx->egl_interface = (lv_egl_interface_t) {
        ...
        /* Render without any display server. This is the platform enum, not glad's
         * GLAD_EGL_MESA_platform_surfaceless extension-presence flag. */
        .egl_platform = EGL_PLATFORM_SURFACELESS_MESA,
        .select_config = select_config_cb,
        .flip_cb = flip_cb,
        /* A pbuffer replaces the window surface, so no native window is involved.
         * The default eglDestroySurface() is the right destructor for it. */
        .create_surface_cb = create_surface_cb,
        .destroy_surface_cb = NULL,
    ...
    /* The EGL context is current from here on, so GL calls are allowed. */
...
lv_result_t lv_opengles_pbuffer_read_to_draw_buf(lv_display_t * display, lv_draw_buf_t * draw_buf)
{
    ...
    /* OpenGL's origin is bottom-left and LVGL's is top-left, so the rows are read back
     * in reverse. Going row by row also keeps any draw buffer stride intact without
     * needing GL_PACK_ROW_LENGTH, which is not available on OpenGL ES 2. */
    for(int32_t y = 0; y < h; y++) {
        uint8_t * row = lv_draw_buf_goto_xy(draw_buf, 0, h - 1 - y);
        GL_CALL(glReadPixels(0, y, w, 1, GL_RGBA, GL_UNSIGNED_BYTE, row));

        /* GL returns RGBA byte order, while LVGL's XRGB8888/ARGB8888 are BGRA in
         * memory on little-endian targets. Swap the red and blue channels. */
        uint8_t * px = row;
...
void lv_opengles_pbuffer_destroy(lv_opengles_pbuffer_t * pbuffer)
{
    ...
    /* Release the GL objects and the OpenGL ES driver's own state while the context is
     * still current, then tear the EGL context down. */
    lv_opengles_deinit();
...
static lv_result_t init_render_target(lv_pbuffer_ctx_t * ctx)
{
     ...
     * glReadPixels on a multisampled pbuffer (GL_INVALID_OPERATION), and the FBO
     * carries its own stencil buffer for NanoVG's clipping regardless of the config.
     *
     * The color attachment is single-sampled, so any MSAA offered by the EGL config
     * does not apply here; antialiasing comes from the draw unit's own geometry AA.
     * Hardware MSAA would need a multisampled renderbuffer plus a blit-resolve into
     * this texture, which is OpenGL ES 3 only. */
...
static size_t select_config_cb(void * driver_data, const lv_egl_config_t * configs, size_t config_count)
{
    ...
    for(size_t i = 0; i < config_count; ++i) {
        ...
        /* Rendering and readback both happen through the FBO, so the only thing wanted
         * from the surface itself is a plain 8 bit per channel RGB format. Multisampling
         * is deliberately not requested: it would not reach the single-sampled FBO, and
         * Mesa then rejects glReadPixels on the pbuffer. */

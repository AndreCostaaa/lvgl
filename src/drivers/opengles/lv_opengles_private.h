...
typedef struct {
    ...
    lv_color_format_t cf;   /**< color format of `texture`. Defaults to `LV_COLOR_FORMAT_NATIVE` */
...
/**
 * Tell whether the red and blue channels of a color format have to be swapped while rendering.
 * The channel order of the LVGL color formats doesn't always match the one OpenGL uploads them with.
 * @param cf        the LVGL color format
 * @return          true: the shader has to swap the red and blue channels
 */
bool lv_opengles_color_format_rb_swap(lv_color_format_t cf);

/**
 * Upload a pixel buffer into an OpenGL texture, (re)defining its size and format
 * @param texture_id    the OpenGL texture ID to upload into
 * @param buf           the pixel buffer. Can be `NULL` to only define the size and format
 ...
 * The size and color format are taken from the display.
 * @param texture_id    the OpenGL texture ID to upload into

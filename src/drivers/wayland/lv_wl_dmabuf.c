...
static struct buffer * lv_wayland_dmabuf_create_draw_buffers_internal(struct window * window)
{
    ...
    for(int i = 0; i < LV_WAYLAND_BUF_COUNT; i++) {
        ...
        buffers[i].buf_base[0]   = buffers->lv_draw_buf->data;
        ...
        zwp_linux_buffer_params_v1_add(params, buffers[i].dmabuf_fds[0], 0, buffers->offsets[0], buffers->strides[0], 0,
                                       0);

        zwp_linux_buffer_params_v1_add_listener(params, &params_listener, buffers);

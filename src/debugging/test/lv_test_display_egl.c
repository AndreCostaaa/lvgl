...
static int32_t fixed_hor_res;
static int32_t fixed_ver_res;
...
lv_display_t * lv_test_display_egl_create(int32_t hor_res, int32_t ver_res)
{
    ...
    fixed_hor_res = hor_res;
    fixed_ver_res = ver_res;

    /* The FBO and the readback path are fixed to the resolution and the 32bpp BGRA
     * layout used at creation. Guard against tests that mutate either on the default
     * display, which would silently desync them and corrupt screenshots. */
     ...
     * draw unit. Both need an active GL context, which the pbuffer display has made
     * current. The driver state is what the NanoVG 3D task uses to blend an
     * lv_3dtexture (e.g. a glTF view) into the layer. */
    lv_opengles_init();
...
static void egl_resolution_changed_cb(lv_event_t * e)
{
    ...
    LV_ASSERT_FORMAT_MSG(hor_res == fixed_hor_res && ver_res == fixed_ver_res,
                         ...
                         fixed_hor_res, fixed_ver_res, hor_res, ver_res);
}

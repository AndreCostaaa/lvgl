...
extern "C" void lv_fs_arduino_esp_littlefs_init(void)
{
	...
	lv_fs_drv_t *fs_drv =
		&(LV_GLOBAL_DEFAULT()->arduino_esp_littlefs_fs_drv);
...
static void *fs_open(lv_fs_drv_t *drv, const char *path, lv_fs_mode_t mode)
{
	...
	lv_snprintf(buf, sizeof(buf), LV_FS_ARDUINO_ESP_LITTLEFS_PATH "%s",
		    path);
...
#if defined(LV_FS_ARDUINO_ESP_LITTLEFS_LETTER) && \
	LV_FS_ARDUINO_ESP_LITTLEFS_LETTER != '\0'
#warning \
	"LV_USE_FS_ARDUINO_ESP_LITTLEFS is not enabled but LV_FS_ARDUINO_ESP_LITTLEFS_LETTER is set"
#endif

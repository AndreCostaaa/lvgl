...
#define LV_LABEL_HINT_HEIGHT_LIMIT \
	1024 /*Enable "hint" to buffer info about labels larger than this. (Speed up drawing)*/
...
static void calculate_x_coordinate(int32_t *x, const lv_text_align_t align,
				   const char *txt, uint32_t length,
				   const lv_font_t *font, int32_t letter_space,
				   lv_area_t *txt_coords, lv_text_flag_t flags);
...
const lv_obj_class_t lv_label_class = {
	...
	.names_count =
		sizeof(lv_label_property_names) / sizeof(lv_property_name_t),
#endif
...
void lv_label_set_long_mode(lv_obj_t *obj, lv_label_long_mode_t long_mode)
{
	...
	if (long_mode == LV_LABEL_LONG_MODE_SCROLL ||
			     ...
			     lv_point_t *pos)
{
	...
	while (txt[new_line_start] != '\0') {
		bool last_line =
			y + letter_height + line_space + letter_height > max_h;
		if (last_line && label->long_mode == LV_LABEL_LONG_MODE_DOTS)
			flag |= LV_TEXT_FLAG_BREAK_ALL;

		new_line_start += lv_text_get_next_line(&txt[line_start],
							LV_TEXT_LEN_MAX, font,
							letter_space, max_w,
							NULL, flag);
	...
	if (base_dir == LV_BASE_DIR_AUTO)
		base_dir = lv_bidi_detect_base_dir(txt);
	...
	if (new_line_start == byte_id) {
		...
		uint32_t visual_char_pos = lv_bidi_get_visual_pos(
			&txt[line_start], &mutable_bidi_txt,
			new_line_start - line_start, base_dir, line_char_id,
			&is_rtl);
		...
		visual_byte_pos =
			lv_text_encoded_get_byte_id(bidi_txt, visual_char_pos);
	}
#else
	bidi_txt = &txt[line_start];
	...
	int32_t x = lv_text_get_width_with_flags(bidi_txt, visual_byte_pos,
						 font, letter_space, flag);
	if (char_id != line_start)
		x += letter_space;

	uint32_t length = new_line_start - line_start;
	calculate_x_coordinate(&x, align, bidi_txt, length, font, letter_space,
			       &txt_coords, flag);
	pos->x = x;
	pos->y = y;
				...
				bool bidi)
{
	...
	if (bidi) {
		...
		lv_bidi_process_paragraph(
			txt + line_start, bidi_txt, txt_len,
			lv_obj_get_style_base_dir(obj, LV_PART_MAIN), NULL, 0);
	} else
#endif
	{
	...
	/*Calculate the x coordinate*/
	int32_t x = 0;
	const lv_text_align_t align = lv_obj_calculate_style_text_align(
	...
	if (new_line_start > 0) {
		while (i + line_start < new_line_start) {
			...
			lv_text_encoded_letter_next_2(bidi_txt, &letter,
						      &letter_next, &i);

			if ((flag & LV_TEXT_FLAG_RECOLOR) != 0) {
				if (lv_text_is_cmd(&cmd_state, bidi_txt[i]) !=
				    false) {
					continue; /*Skip the letter if it is part of a command*/
				}
			}

			int32_t gw = lv_font_get_glyph_width(font, letter,
							     letter_next);

			/*Finish if the x position or the last char of the next line is reached*/
			if (pos.x < x + gw ||
			    i + line_start == new_line_start ||
	...
	if (bidi) {
		...
		if (txt[line_start + i] == '\0') {
			...
			bool is_rtl;
			logical_pos = lv_bidi_get_logical_pos(
...
bool lv_label_is_char_under_pos(const lv_obj_t *obj, lv_point_t *pos)
{
	...
	if (align == LV_TEXT_ALIGN_CENTER) {
		const int32_t line_w = lv_text_get_width_with_flags(
			&txt[line_start], new_line_start - line_start, font,
			letter_space, flag);
	...
	if (new_line_start > 0) {
		while (i <= new_line_start - 1) {
			/*Get the current letter and the next letter for kerning*/
			/*Be careful 'i' already points to the next character*/
			lv_text_encoded_letter_next_2(txt, &letter,
						      &letter_next, &i);

			if ((flag & LV_TEXT_FLAG_RECOLOR) != 0) {
				if (lv_text_is_cmd(&cmd_state, txt[i]) !=
				    false) {
					continue; /*Skip the letter if it is part of a command*/
				}
			}
...
static void lv_label_event(const lv_obj_class_t *class_p, lv_event_t *e)
{
	...
	if ((code == LV_EVENT_STYLE_CHANGED) ||
		...
		if (label->invalid_size_cache) {
			...
			lv_text_get_size(&label->size_cache, label->text, font,
					 letter_space, line_space, w, flag);
...
static void draw_main(lv_event_t *e)
{
	...
	if ((label->long_mode == LV_LABEL_LONG_MODE_SCROLL ||
		...
		lv_point_t size;
		lv_text_get_size(&size, label->text, label_draw_dsc.font,
				 label_draw_dsc.letter_space,
				 label_draw_dsc.line_space, LV_COORD_MAX, flag);
	...
	lv_area_t txt_clip;
	bool is_common =
		lv_area_intersect(&txt_clip, &txt_coords, &layer->_clip_area);
	if (!is_common) {
		return;
	}
...
static void overwrite_anim_property(lv_anim_t *dest, const lv_anim_t *src,
...
static void lv_label_refr_text(lv_obj_t *obj)
{
	...
	lv_label_revert_dots(obj);
	lv_text_get_size(&size, label->text, font, letter_space, line_space,
			 max_w, flag);
	...
	if (label->long_mode == LV_LABEL_LONG_MODE_SCROLL) {
		...
		uint32_t anim_time =
			lv_obj_get_style_anim_duration(obj, LV_PART_MAIN);
		if (anim_time == 0)
		...
		if (size.x > lv_area_get_width(&txt_coords)) {
			...
			if (anim_cur) {
				...
				reverse_play_in_progress =
					anim_cur->reverse_play_in_progress;
			}
			if (act_time < a.duration) {
				a.act_time =
					act_time; /*To keep the old position*/
				a.early_apply = 0;
			...
			if (anim_template)
				overwrite_anim_property(&a, anim_template,
							label->long_mode);
			lv_anim_start(&a);
			hor_anim = true;
		} else {
	...
	/*In roll inf. mode keep the size but start offset animations*/
		...
		if (size.x > lv_area_get_width(&txt_coords)) {
			...
			if (anim_template) {
				...
				a.act_time =
					act_time; /*To keep the old position when the label text is updated mid-scrolling*/
				a.early_apply = 0;
			}
		...
		if (size.y >
			    lv_area_get_height(
				    &txt_coords) && /*Text overflows available area*/
		    size.y >
			    lv_font_get_line_height(
				    font) && /*No break requested, so no dots required*/
		    lv_text_get_encoded_length(label->text) >
			    LV_LABEL_DOT_NUM) { /*Do not turn all characters into dots*/
			lv_point_t p;
			int32_t y_overed;
			...
			uint32_t letter_id =
				lv_label_get_letter_on(obj, &p, false);

			/*Be sure there is space for the dots*/
			size_t txt_len = lv_strlen(label->text);
			uint32_t byte_id = lv_text_encoded_get_byte_id(
	...
	} else if (label->long_mode == LV_LABEL_LONG_MODE_CLIP ||
		   label->long_mode == LV_LABEL_LONG_MODE_WRAP) {
		/*Do nothing*/
	}
...
static void lv_label_revert_dots(lv_obj_t *obj)
{
	...
	if (label->dot_begin != LV_LABEL_DOT_BEGIN_INV) {
		for (int i = 0; i < LV_LABEL_DOT_NUM + 1 && label->dot[i];
		     i++) {
...
{
	lv_label_t *label = (lv_label_t *)obj;
	LV_ASSERT_MSG(label->dot_begin == LV_LABEL_DOT_BEGIN_INV,
		      "Label dots already set");
	if (dot_begin != LV_LABEL_DOT_BEGIN_INV) {
		/*Save characters*/
		lv_strncpy(label->dot, &label->text[dot_begin],
			   LV_LABEL_DOT_NUM + 1);
		label->dot_begin = dot_begin;

		/*Overwrite up to LV_LABEL_DOT_NUM + 1 characters with dots and null terminator*/
		int i = 0;
		for (; i < LV_LABEL_DOT_NUM && label->text[dot_begin + i];
		     i++) {
				   ...
				   lv_area_t *txt_coords, lv_text_flag_t flags)
{
	if (align == LV_TEXT_ALIGN_CENTER) {
		const int32_t line_w = lv_text_get_width_with_flags(
			txt, length, font, letter_space, flags);
		*x += lv_area_get_width(txt_coords) / 2 - line_w / 2;
	} else if (align == LV_TEXT_ALIGN_RIGHT) {
		const int32_t line_w = lv_text_get_width_with_flags(
			txt, length, font, letter_space, flags);

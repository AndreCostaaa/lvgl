...
#define ASSET_PATH(name) "src/test_assets/gltf/" name
...
{
    size_t size = 0;
    uint8_t * bytes = read_asset(ASSET_PATH("minimal_triangle.glb"), &size);
...
{
    size_t size = 0;
    uint8_t * bytes = read_asset(ASSET_PATH("minimal_triangle.gltf"), &size);

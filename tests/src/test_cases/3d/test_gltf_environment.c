...
/* Fingerprint of the rendered area, to tell whether the lighting changed */
static uint32_t area_checksum(const lv_area_t * area)
{
    ...
    for(int32_t y = area->y1; y <= area->y2; y++) {
        for(int32_t x = area->x1; x <= area->x2; x++) {
            const uint8_t * px = lv_draw_buf_goto_xy(buf, x, y);
            sum = sum * 31u + px[0] + px[1] + px[2];
        }
    }
    return sum;
}
...
void test_gltf_environment_create_from_native_path(void)
{
    ...
    lv_gltf_environment_t * env = lv_gltf_environment_create(sampler, ENV_IMAGE_NATIVE("environment.hdr"));
    TEST_ASSERT_NOT_NULL(env);
...
void test_gltf_environment_image_is_used(void)
{
    ...
    render_frames();
    uint32_t with_default = area_checksum(&area);

    lv_gltf_set_environment(gltf, file_env);
    lv_obj_invalidate(gltf);
    render_frames();
    uint32_t with_file = area_checksum(&area);

    TEST_ASSERT_NOT_EQUAL(with_default, with_file);
...
/* Two environments with different rotations produce different images */
void test_gltf_environment_angle_changes_the_image(void)
{
    lv_gltf_ibl_sampler_t * sampler = lv_gltf_ibl_sampler_create();

 ...
 *
 * The rendering itself goes into a framebuffer object rather than into the pbuffer:
 * the pbuffer only exists so the EGL context can be made current, and reading back a
 * multisampled pbuffer is not portable (Mesa rejects it). The FBO also supplies the
 * stencil buffer NanoVG needs for clipping, independently of the EGL config.
 *
 * The resolution and color format are fixed at creation; changing either on the
 * returned display is not supported.
 *
 * This expects a draw unit that renders through the bound GL framebuffer, i.e. NanoVG.
 * The OpenGL ES draw unit renders into the display's own texture instead, so it would
 * additionally need that texture composited into the FBO before the readback; that path
 * is not implemented.
 *

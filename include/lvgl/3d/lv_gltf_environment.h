 ...
 *       letter like any other LVGL path. Paths the file system cannot resolve are
 *       passed on to the image loader directly.
 * @note An image that cannot be read is reported with a warning and the default
 *       embedded image is used instead

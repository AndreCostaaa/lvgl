...
The Linux event device (evdev) interface exposes input events — from a touchscreen, mouse, keyboard or
anything else the kernel recognizes — through device files under `/dev/input/`. The evdev driver reads them
directly, so it needs no user-space input library at all.
...
display drivers, neither of which brings its own input. If you need full keyboard layouts and modifier
handling, or device-specific quirks, use [libinput](/integration/embedded_linux/drivers/libinput) instead.
...
Enabling it is what pulls the `evdev` library into the build; LVGL's CMake integration resolves it for you.
It cannot be built from source by LVGL, so it must already be present on the system or in your sysroot. See

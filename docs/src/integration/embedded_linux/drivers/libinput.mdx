...
libinput is the input stack used by most Linux desktops. It covers mice, keyboards, touchpads, touchscreens
and graphics tablets, absorbing device-specific quirks and normalizing everything into a uniform event
stream.
...
keyboards — through XKB. In exchange it adds a dependency and gives up automatic device discovery. Use evdev
for a minimal system with a known touchscreen; use libinput when you need proper text entry or a device with
quirks.
...
Enabling it is what pulls `libinput` into the build, and `xkbcommon` too when XKB support is on — LVGL's
CMake integration resolves both. Neither can be built from source by LVGL, so they must already be present on
the system or in your sysroot. See [Dependency Management](/integration/building/cmake).
...
<Callout type="warning" title="LV_LIBINPUT_XKB_KEY_MAP is deprecated">
The single `LV_LIBINPUT_XKB_KEY_MAP` struct that older versions used is deprecated and will be removed in the
next release; setting it produces a build warning. Use the individual options above instead.
</Callout>
...
| Automatic discovery | No — create devices explicitly, optionally located by capability |
| Hotplug | No — use [evdev](/integration/embedded_linux/drivers/evdev) if you need it |
| Full keyboard (layouts, modifiers) | Yes — enable `LV_LIBINPUT_XKB` |
| Touch calibration | Not needed — libinput normalizes coordinates itself |
| Axis swap | No — handle it with libinput quirks or udev rules |
| Find device by capability | Yes — <ApiLink name="lv_libinput_find_dev" /> and <ApiLink name="lv_libinput_find_devs" /> |
| BSD | Yes — enable `LV_LIBINPUT_BSD` |

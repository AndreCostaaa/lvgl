...
Because the context already exists, LVGL's GPU draw units work out of the box, which makes GLFW the quickest
way to get hardware-accelerated LVGL running on a PC-like platform.

It is an alternative to the [Wayland](/integration/embedded_linux/drivers/wayland),
[X11](/integration/embedded_linux/drivers/X11) and [SDL](/integration/pc/sdl) drivers for development. It can
also composite several LVGL displays and externally-supplied OpenGL textures into one window, which is useful
when embedding LVGL alongside other OpenGL content.

## Configuration

Enable <ApiLink name="LV_USE_GLFW" />. It turns on <ApiLink name="LV_USE_OPENGLES" /> for you.

<Callout type="warning" title="GLFW and EGL are mutually exclusive">
<ApiLink name="LV_USE_GLFW" /> cannot be combined with <ApiLink name="LV_USE_EGL" /> — GLFW manages the
context itself, so the two window-system layers would collide. If you need EGL, use the
[EGL driver](/integration/embedded_linux/drivers/egl) instead of GLFW.
</Callout>

Enabling the driver is what pulls GLFW into the build. Unlike most Linux display dependencies, GLFW **can**
be fetched and built from source by LVGL if it is not installed, so you do not have to provide it yourself.
...
<ApiLink name="lv_opengles_glfw_window_create_ex" /> is the same call with a window title and horizontal /
vertical mirroring; <ApiLink name="lv_opengles_glfw_window_set_title" /> and
<ApiLink name="lv_opengles_glfw_window_set_flip" /> change them later.
<ApiLink name="lv_opengles_glfw_window_get_glfw_window" /> returns the underlying `GLFWwindow *` if you need
to call GLFW directly. <ApiLink name="lv_opengles_window_delete" /> closes a window — deleting the last one
exits the process.
...
| Capability | Value |
|---|---|
| Rotation | Hardware (GPU) — all four orientations |
| Runtime resolution change | No |
| Runtime color format change | No |
| Color formats | Derived from <ApiLink name="LV_COLOR_DEPTH" /> |
| Multiple displays / windows | Yes — multiple windows, and multiple displays or textures per window |
| Hardware acceleration | OpenGL ES, via <ApiLink name="LV_USE_DRAW_NANOVG" /> or <ApiLink name="LV_USE_DRAW_OPENGLES" /> |
| Render mode | Fixed by the driver — Full with <ApiLink name="LV_USE_DRAW_OPENGLES" />, otherwise Direct |
| Input | Built in — mouse per window, keyboard |

The GPU owns the framebuffer, so the render mode is chosen by the driver and there is no knob to turn.
